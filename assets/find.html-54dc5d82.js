import{_ as n,W as a,X as s,Y as e}from"./framework-9a5142fa.js";const i={},l=e(`<h2 id="find-解析" tabindex="-1"><a class="header-anchor" href="#find-解析" aria-hidden="true">#</a> find 解析</h2><p><code>find path [options] [expression] </code></p><p>path 默认当前路径</p><p>​ 查找指定路径及其内部的所有子目录。</p><p>options</p><p>​ -name 按照文件名，支持通配符</p><p>​ -perm 按照文件权限来查找文件</p><p>​ -type 查找某一类型的文件</p><p>​ f(普通文件从<code>-</code>变成<code>f</code>),d(文件夹) ​ b,c ​ l,p,s</p><p>​ -size选项 ,按文件的大小查找文件的</p><p>​ -mtime -n +n 按照文件的更改时间来查找文件，-n表示文件更改时间距现在n天以内，+n表示文件更改时间距现在n天以前。</p><p>​ -newer file1 查找更改时间比文件file1新的文件。</p><p>expression：默认-print</p><p>​ -print find命令将匹配的文件输出到标准输出。</p><p>​ -exec find命令对匹配的文件执行该参数所给出的shell命令。</p><p>​ -ok 和- exec的作用相同，只不过以一种更为安全的模式来执行该参数所给出的shell命令，在执行每一个命令之前，提示用户是否执行.</p><h2 id="指定名字" tabindex="-1"><a class="header-anchor" href="#指定名字" aria-hidden="true">#</a> 指定名字</h2><p>当前目录下指定名字</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">find</span> /usr <span class="token parameter variable">-type</span> f <span class="token parameter variable">-name</span> <span class="token string">&quot;Bash&quot;</span>

<span class="token comment"># 不区分大小写 -iname</span>
...
<span class="token comment"># 逻辑或 -o</span>
...
<span class="token function">find</span> <span class="token parameter variable">-type</span> f <span class="token parameter variable">-name</span> <span class="token string">&quot;*.md&quot;</span> <span class="token operator">|</span> <span class="token function">xargs</span> <span class="token function">rm</span> <span class="token parameter variable">-rf</span> 
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>- excec</code></p><ul><li><code>{}</code> 大括号充当匹配文件结果的占位符，因此 <code>rm-v {}</code> 将删除find命令找到的文件；</li><li>结束 shell 执行的命令（exec之后的命令） <ul><li>第一种是<code>\\;</code>。反斜杠 <code>\\</code>是转义分号。</li><li>第二种是<code>+</code></li></ul></li></ul><p><code>xargs</code></p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># 多行输入单行输出：</span>
...
<span class="token comment"># 假如你有一个文件包含了很多你希望下载的 URL，你能够使用 xargs下载所有链接</span>
<span class="token function">cat</span> url-list.txt <span class="token operator">|</span> <span class="token function">xargs</span> <span class="token function">wget</span> <span class="token parameter variable">-c</span>

<span class="token comment"># 查找所有的 jpg 文件，并且压缩它们：</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,32),p=[l];function c(t,r){return a(),s("div",null,p)}const d=n(i,[["render",c],["__file","find.html.vue"]]);export{d as default};
